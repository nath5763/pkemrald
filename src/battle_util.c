...
{
    return gBattleMons[battler].ability == ABILITY_LEAF_GUARD
        && WEATHER_HAS_EFFECT
            ...
            case ENDTURN_YAWN:  // yawn
                if (gStatuses3[gActiveBattler] & STATUS3_YAWN)
                {
                    ...
                    if (!(gStatuses3[gActiveBattler] & STATUS3_YAWN) && !(gBattleMons[gActiveBattler].status1 & STATUS1_ANY)
                     ...
                     && !IsLeafGuardProtected(gActiveBattler) && !UproarWakeUpCheck(gActiveBattler))
                    {
                        CancelMultiTurnMoves(gActiveBattler);
        ...
        case ABILITYEFFECT_ON_SWITCHIN: // 0
            ...
            case ABILITY_SNOW_CLOAK:
                if (!(gBattleWeather & B_WEATHER_HAIL_PERMANENT))
                {
                    ...
                    BattleScriptPushCursorAndCallback(BattleScript_SnowCloakActivates);
                    gBattleScripting.battler = battler;
                    effect++;
                }
                break;
